...
超参数
'''
MAX_EPISODE = 30  # 最大训练的episode数
MAX_EPISODE_STEP = 20000  # 每一个episode的step数
N_WORKER = 1  # worker的个数
GAMMA = 0.9  # 奖励的折扣
A_LR = 0.003  # actor的learning rate
C_LR = 0.005  # critic的learning rate
MIN_BATCH_SIZE = 32  # 更新ppo用的最小的batch size
...
class PPO(object):
    ...
    def _build_cnet(self):
        ...
        c_x = Dense(4, activation='relu')(c_input)
        #c_x = Dense(4, activation='relu')(c_x)
        c_v = Dense(1)(c_x)
    ...
    def _build_anet(self, name):
        ...
        a_x = Dense(4, activation='relu')(a_input)
        #a_x = Dense(8, activation='relu')(a_x)
    ...
    def a_train(self, observation, action, advantage):
        ...
        with tf.GradientTape() as tape:
            ...
            a_loss = -tf.reduce_mean(tf.minimum(surr, tf.clip_by_value(ratio, 1.0-EPSILON, 1.0+EPSILON)*advantage)) - 0.05* entropy
    ...
    def choose_action(self, observation):
        ...
        logits = self.actor(observation)
        print('logits',logits)
        dist_masked = tfp.distributions.Categorical(logits=logits)  # 构建类别分布，
        print('dist',dist_masked)
        a_masked = dist_masked.sample()  # 根据概率随机出动作
        print('a_masked',a_masked)
        a_masked = a_masked.numpy()[0]
        print('<TRAIN_LOG> ','action',a_masked)
    ...
    def load_model_weights(self):
        self.critic.load_weights('ppo_discrete_critic.h5')
        self.actor.load_weights('ppo_discrete_actor.h5')
        self.actor_old.load_weights('ppo_discrete_actor_old.h5')
        ...
        while ns.coord_status:
            try:
                ...
                for t in range(MAX_EPISODE_STEP):
                    ...
                    print('<TRAIN_LOG> ', ' episode: ', ns.GLOBAL_EPISODE, '/', MAX_EPISODE, '  Worker:', self.wid,' reward:',reward, ' action:', action, ' ob:', observation, 'ob_:',observation_,  'step', t, ' done:', done, 'pid:',os.getpid(), ' time:', datetime.now())
                    ...
                    if (t == MAX_EPISODE_STEP - 1) or (ns.GLOBAL_UPDATE_COUNTER >= MIN_BATCH_SIZE) or done:
                        ...
                        if ns.GLOBAL_EPISODE >= MAX_EPISODE:  # stop training
                            running_reward = pd.Series(ns.GLOBAL_RUNNING_REWARD)
                            print('running_reward',ns.GLOBAL_RUNNING_REWARD)
                            running_reward.to_csv('running_reward.csv')
                            ROLLING_EVENT.set()
                            UPDATE_EVENT.set()
